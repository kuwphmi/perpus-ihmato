      ...
      const res = await fetch(
        "http://localhost:3000/api/loan-requests",
        {
          ...
          body: JSON.stringify({
            user_id: user.id,
            book_key:
              selectedBook.cover + "_" + selectedBook.title,
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {categories.map((item, i) => (
            <div
              ...
              className={`bg-white p-5 rounded-xl shadow hover:shadow-md transition text-center cursor-pointer ${activeCategory === item.name
