...
import { FiSearch, FiBell, FiHeart, FiHome, FiBook, FiShoppingCart, FiClock } from "react-icons/fi";
...
export default function Riwayat() {
  ...
  useEffect(() => {
    ...
    const filtered = historyBooks.filter((book) => book.title?.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleExtension = async (book) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/extensions/request`, {
        method: "POST",
        headers: {
      ...
      if (res.ok && result.status) {
        showNotif("Extension request sent successfully", "success");
        await fetchHistory();
      } else {
        const errorMessage = result.message || "Unable to request extension";
        showNotif(errorMessage, "error");
      }
    } catch (err) {
      console.error(err);
      showNotif("Failed to send extension request", "error");
    } finally {
      setIsRequestingExtension(false);
    }
  };

  const openConfirmExtension = (book) => {
    setExtensionTarget(book);
    setShowExtendConfirm(true);
  };

  const closeConfirmExtension = () => {
  ...
  const confirmExtension = async () => {
    ...
    await handleExtension(extensionTarget);
    closeConfirmExtension();
  };
        ...
        <div className="max-w-6xl mx-auto flex items-center px-6 py-2">
          ...
          <div className="flex-1 flex justify-center">
            <div className="relative w-full max-w-lg">
              ...
              <input type="text" placeholder="Search Books..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-full focus:outline-blue-500" />
            </div>
          </div>
...
"
      ...
      "
                ...
                {/* INFO */}
                <div>
                  <h3 className="font-bold text-sm md:text-lg text-gray-800 line-clamp-1">{book.title}</h3>

                  <p className="text-sm text-gray-500">{book.loan_date ? new Date(book.loan_date).toLocaleDateString("id-ID") : "-"}</p>
      ...
      `}
                    >
                      {book.status === "pending" ? "Pending" : book.status === "approved" || book.status === "borrowed" ? "Approved" : book.status === "returned" ? "Returned" : "Rejected"}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2 w-27.5 ml-auto">
      ...
      "
                  ...
                  {book.status !== "returned" && (
                    <button
                      onClick={() => openConfirmExtension(book)}
                      className="
      ...
      "
        ...
        {showExtendConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8">
            <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Confirm Extension</h3>
              <p className="text-sm text-gray-600 mb-6">
                Are you sure you want to extend <span className="font-semibold text-gray-800">{extensionTarget?.title}</span>? This will send a request to extend the loan for 7 more days.
              </p>
              <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
                <button type="button" onClick={closeConfirmExtension} className="w-full sm:w-auto px-4 py-3 rounded-2xl border border-gray-200 text-gray-700 hover:bg-gray-50 transition">
                  Cancel
                </button>
                <button
                  type="button"
                  ...
                  className="w-full sm:w-auto px-4 py-3 rounded-2xl bg-yellow-500 text-white hover:bg-yellow-600 transition disabled:cursor-not-allowed disabled:bg-yellow-300"
                >
                  {isRequestingExtension ? "Sending..." : "Yes, Extend"}
                </button>
              </div>
            </div>
          </div>
        ...
        {/* CONTENT */}
        <main className="flex-1 bg-white">{/* semua isi halaman */}</main>
      ...
      <footer className="mt-auto bg-gray-900 text-white">
        ...
        {/* BOTTOM */}
        <div className="border-t border-gray-800 py-4 text-center text-sm text-gray-500">© 2026 BookIn. All rights reserved.</div>
      </footer>
    </div>
  );
}