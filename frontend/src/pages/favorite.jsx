...
export default function Favorit() {
  ...
  const fetchDescription = async (workKey, book) => {
    try {
      ...
      if (typeof data.description === "string") {
        ...
        setBookDescription(`${book.title} is written by ${book.author}.`);
      }
    } catch (err) {
      if (book.firstSentence) {
        setBookDescription(book.firstSentence);
      } else {
        setBookDescription(`No detailed description available.`);
      }
    }
  };
  ...
  return (
    ...
    "
          ...
          <div>
            <h1 className="text-[20px] font-semibold text-blue-600">My Favorites</h1>
          </div>
        </div>
  ...
  "
      ...
      <div className="px-5 py-3">
        <div className="flex items-center bg-white rounded-full px-4 py-2 shadow-sm border border-gray-200 focus-within:ring-2 focus-within:ring-blue-400 transition">
          ...
          <input type="text" placeholder="Search favorites..." className="outline-none text-sm w-full bg-transparent" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      <div className="px-5 pb-10">
        {filtered.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {filtered.map((book) => (
              <div key={book.workKey} className="group bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
                {/* COVER */}
                <div className="relative h-44 md:h-60 bg-linear-to-br from-blue-50 to-blue-100 flex items-center justify-center overflow-hidden">
                  ...
                  {book.cover ? <img src={book.cover} alt={book.title} className="h-full w-full object-cover" /> : <div className="text-gray-400 text-sm">No Cover</div>}

                  {/* LOVE BUTTON */}
                  <button onClick={() => removeFavorite(book.workKey)} className="absolute top-3 right-3 z-20 bg-white/90 p-2 rounded-full shadow hover:scale-110 transition">
                    <Heart className="w-4 h-4 text-red-500 fill-red-500" />
                  </button>
                </div>

                {/* CONTENT */}
                <div className="p-4 flex flex-col h-47.5">
                  <div>
                    ...
                    <p className="text-gray-400 text-xs mt-1 mb-3 line-clamp-2 min-h-8">{book.firstSentence || `This book discusses ${book.subjects?.join(", ") || "various topics"}.`}</p>
                  </div>

                  {/* BUTTON */}
                  <div className="flex gap-2 mt-auto">
                    <button onClick={() => handleBorrow(book)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl text-xs md:text-sm font-semibold transition">
                      Borrow
                    </button>
          ...
          <>
            ...
            <div className="text-center mt-24">
              ...
              <div className="flex flex-col items-center justify-center mt-6 text-center">
                ...
                <p className="text-gray-400 text-sm mb-4">Save the books you love so they’re easier to find later.</p>

                <button onClick={() => navigate("/koleksi")} className="bg-blue-600 text-white px-5 py-2 rounded-lg shadow hover:bg-blue-700 transition">
                  Explore Books
                </button>
              </div>
            </div>
      ...
      {showBorrowPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowBorrowPopup(false)}>
          <div className="bg-white p-5 rounded-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-4 text-gray-700">Your borrowing request will be sent to the admin.</p>

            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowBorrowPopup(false)} className="px-4 py-2 bg-gray-200 rounded-lg">
                Cancel
              </button>

              <button onClick={submitLoanRequest} className="px-5 py-2 bg-blue-600 text-white rounded-lg">
                Borrow
              </button>
            </div>
          </div>
      ...
      {showDetailPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowDetailPopup(false)}>
          <div className="bg-white p-5 rounded-2xl w-[90%] max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            {/* COVER */}
            <div className="w-full h-64 bg-blue-100 rounded-xl overflow-hidden mb-4 flex items-center justify-center">
              {selectedBook?.cover ? <img src={`https://covers.openlibrary.org/b/id/${selectedBook.cover}-L.jpg`} alt={selectedBook?.title} className="w-full h-full object-cover" /> : <p>No Cover</p>}
            </div>
            ...
            <div className="flex justify-between items-start gap-3">
              ...
              <button onClick={() => removeFavorite(selectedBook?.workKey)} className="bg-red-100 p-2 rounded-full">
            ...
            {/* DESC */}
            <p className="mt-5 text-sm text-gray-600 leading-relaxed">{bookDescription}</p>
          </div>
        </div>
      )}
