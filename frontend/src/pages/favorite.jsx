...
export default function Favorit() {
  ...
  // Mengambil data user (Pastikan localStorage 'user' sudah diset saat login)
  const user = JSON.parse(localStorage.getItem("user")) || {};
  const popupRef = useRef();
  const navigate = useNavigate();

  // Load data favorit dari localStorage saat komponen dimuat
  useEffect(() => {
    const data = JSON.parse(localStorage.getItem("favorites")) || [];
    setFavorites(data);
  }, []);

  // Handle klik di luar pop-up profile untuk menutupnya
  useEffect(() => {
  ...
  // Membuka popup detail buku (Menggunakan deskripsi yang sudah tersimpan di data favorit)
  const handleDetail = (book) => {
    setSelectedBook(book);
    setBookDescription(book.description || "No description available for this book.");
    setShowDetailPopup(true);
  };

  // Membuka popup konfirmasi peminjaman
  const handleBorrow = (book) => {
    setSelectedBook(book);
    setShowBorrowPopup(true);
  };

  // Mengirim request peminjaman ke API backend lokal
    ...
    try {
      const res = await fetch("http://localhost:3000/api/loan-requests", {
        ...
        body: JSON.stringify({
          ...
          cover: selectedBook.cover_url, // Mengirim cover local
        }),
      });
    ...
    <div className="min-h-screen bg-[#f7faff]">
      ...
      <div className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b px-4 md:px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          {/* 👤 PROFILE (DENGAN FOTO ATAU PLACEHOLDER) */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              setIsProfileOpen(!isProfileOpen);
            }}
            className="relative z-50 w-8 h-8 md:w-9 md:h-9 rounded-full bg-red-100 flex items-center justify-center text-white text-sm font-semibold shadow-lg hover:scale-105 transition-all duration-300 border-2 border-white overflow-hidden"
          >
            {/* Jika user punya foto profil, tampilkan. Jika tidak, tampilkan inisial/placeholder */}
            {user.photo || user.profile_picture ? (
              <img src={user.photo || user.profile_picture} alt="Profile" className="w-full h-full object-cover" />
            ) : (
              // Placeholder: Avatar karikatur sederhana
              <div className="w-full h-full rounded-full bg-red-500 flex items-center justify-center overflow-hidden border border-red-600">
                <img src="https://img.icons8.com/?size=100&id=t5EaH6F2mQ9X&format=png&color=FFFFFF" alt="Fallback User" className="w-[75%] h-[75%] object-contain opacity-90" />
              </div>
            )}
          </button>
        </div>
      ...
      {/* 🔹 LIST BUKU (OPTIMAL MOBILE & LOCAL DATA) */}
              ...
              <div key={book.workKey} className="group bg-white rounded-2xl md:rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
                ...
                <div className="p-3 md:p-4 flex flex-col grow justify-between">
                  <div>
                    ...
                    {/* Deskripsi kartu (Sembunyikan di mobile agar rapi) */}
                    <p className="text-gray-400 text-[11px] md:text-xs mt-1 mb-2 line-clamp-2 hidden sm:block">{book.description || "No description available for this book."}</p>
                  </div>
      ...
      {/* 🔥 POPUP PROFILE (SESUAI GAMBAR REFERENSI DENGAN FOTO ATAU FALLBACK) */}
      {isProfileOpen && (
        <>
          ...
          <div ref={popupRef} className="absolute right-4 md:right-6 top-16 z-50 w-64 md:w-72 bg-white rounded-3xl shadow-[0_12px_40px_rgba(0,0,0,0.12)] border border-gray-100 overflow-hidden animate-[fadeIn_.2s_ease]">
            {/* BACKGROUND GRADIENT HEADER (Blue to Cyan) */}
            <div className="h-24 bg-linear-to-r from-blue-500 to-cyan-400 relative">
              ...
              <div className="absolute left-1/2 -bottom-10 -translate-x-1/2">
                <div className="w-20 h-20 rounded-full bg-white p-0.75 shadow-lg overflow-hidden flex items-center justify-center">
                  {user.photo || user.profile_picture ? (
                    <img src={user.photo || user.profile_picture} alt="Profile" className="w-full h-full rounded-full object-cover" />
                  ) : (
                    // Placeholder: Avatar karikatur di dalam pop-up
                    <div className="w-full h-full rounded-full bg-red-100 flex items-center justify-center overflow-hidden border border-red-200">
                      <img src="https://img.icons8.com/?size=100&id=t5EaH6F2mQ9X&format=png&color=000000" alt="Fallback User" className="w-[85%] h-[85%] object-contain opacity-80" />
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* DETAIL POPUP (FIX DESKRIPSI LOKAL MUNCUL) */}
      {showDetailPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowDetailPopup(false)}>
          <div className="bg-white p-5 md:p-6 rounded-2xl w-full max-w-md max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            {/* COVER */}
            <div className="w-full h-56 md:h-64 bg-blue-50 rounded-xl overflow-hidden mb-4 flex items-center justify-center border border-gray-100 shadow-inner">
            ...
            <div className="flex justify-between items-start gap-3">
              <div>
                <h2 className="text-lg md:text-xl font-bold text-gray-800 leading-tight">{selectedBook?.title}</h2>
                <p className="text-xs md:text-sm text-gray-500 mt-1">{selectedBook?.author}</p>
              </div>
              <button
                onClick={() => {
            ...
            {/* DESKRIPSI (Menampilkan data lokal deskripsi secara instan) */}
            ...
            <button onClick={() => setShowDetailPopup(false)} className="mt-6 w-full py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium text-xs md:text-sm rounded-xl transition">
              Close
            </button>
          </div>
        </div>
      ...
      {/* FOOTER */}
      <footer className="mt-20 bg-gray-900 text-white">{/* ... (isi footer sama) */}</footer>
      <Floating />
    </div>
  );
}