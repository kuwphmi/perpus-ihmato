...
export default function Favorit() {
  ...
  const handleDetail = async (book) => {
    ...
    // Jika sudah ada deskripsi di objek book
    if (book.description && book.description.trim() !== "") {
      setBookDescription(book.description);
    }
    // Jika tidak ada deskripsi tetapi ada id (dari database)
    else if (book.id) {
      setBookDescription("Loading description...");
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/buku/${book.id}`);
        const data = await res.json();
        // Sesuaikan dengan struktur response backend Anda
        const desc = data.description || data.data?.description || "No description available for this book.";
